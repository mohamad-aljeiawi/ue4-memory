...
extern const std::vector<std::vector<std::u8string>> fa_AlphabetsAllForms;
...
namespace FarsiType
{
    ...
    unsigned char GetFACharPlace(const std::u8string &fa_character, const std::u8string &prevFAChar, const std::u8string &nextFAChar);
    ...
    bool IsFACharBeginner(const std::u8string &fa_character);
    ...
    unsigned char FindFACharIndex(const std::u8string &fa_character);

    /// <summary>Check if given character is a Farsi/Arabic letter or not.</summary>
    /// <param name="fa_character">Character you want to check.</param>
    /// <returns>Returns a bool value. Possible values: true, false.</returns>
    bool IsFAChar(const std::u8string &fa_character);
    ...
    std::vector<std::u8string> ReverseFAText(const std::u8string &str);
    ...
    std::u8string GetFACharGlyph(const std::u8string &fa_character, const std::u8string &prevFAChar, const std::u8string &nextFAChar);
    ...
    std::u8string ConvertToFAGlyphs(const std::u8string &text);
    char *ConvertToFAGlyphs2(const std::u8string &text);
};