...
const std::vector<std::vector<std::u8string>> fa_AlphabetsAllForms = {
...
unsigned char FarsiType::GetFACharPlace(const std::u8string &fa_character, const std::u8string &prevFAChar, const std::u8string &nextFAChar)
{
    bool in_previous = false;
    bool in_next = false;
    for (auto const &fachar : fa_AlphabetsAllForms)
    {
        if (prevFAChar != u8"ISNOTFA" || fachar[faa_Unicode] == prevFAChar)
            in_previous = true;
        if (nextFAChar != u8"ISNOTFA" || fachar[faa_Unicode] == nextFAChar)
            in_next = true;
    }
...
bool FarsiType::IsFACharBeginner(const std::u8string &fa_character)
{
...
unsigned char FarsiType::FindFACharIndex(const std::u8string &fa_character)
{
...
bool FarsiType::IsFAChar(const std::u8string &fa_character)
{
    for (auto const &fachar : fa_AlphabetsAllForms)
    {
...
std::vector<std::u8string> FarsiType::ReverseFAText(const std::u8string &str)
{
    std::vector<std::u8string> reversedStr;
    std::vector<std::u8string> tempFarsiListStr;
    ...
    for (int i = 0; i < str.size(); i++)
    {
        if (str[i] > ' ' && str[i] <= '~')
        {
            ...
            reversedStr.insert(reversedStr.end(), std::u8string() + str[i]);
        }
        else if (str[i] == ' ')
        {
            if (tempFarsiListStr.size() > 0)
            {
                ...
                {
                    tempFarsiListStr.insert(tempFarsiListStr.begin(), std::u8string() + str[i]);
                }
                else
                {
                    tempFarsiListStr.insert(tempFarsiListStr.end(), std::u8string() + str[i]);
                }
            }
            else
            {
        ...
        {
            tempFarsiListStr.insert(tempFarsiListStr.begin(), std::u8string() + str[i] + str[i + 1]);
            i++;
        }
...
std::u8string FarsiType::GetFACharGlyph(const std::u8string &fa_character, const std::u8string &prevFAChar, const std::u8string &nextFAChar)
{
...
std::u8string FarsiType::ConvertToFAGlyphs(const std::u8string &text)
{
    std::vector<std::u8string> reversed_text = ReverseFAText(text);
    std::u8string convertedText;
    std::u8string previous_fachar, next_fachar;
    ...
    for (int i = 0; i < reversed_text.size(); ++i)
    {
        ...
        if ((i + 1) <= (reversed_text.size() - 1))
        {
            if (IsFAChar(reversed_text[i + 1]))
                ...
                next_fachar = u8"ISNOTFA";
        }
        else
            next_fachar = u8"ISNOTFA";

        std::u8string fa_glyph = GetFACharGlyph(reversed_text[i], next_fachar, previous_fachar);
        convertedText.append(fa_glyph);
    }
    return convertedText;
}

char *FarsiType::ConvertToFAGlyphs2(const std::u8string &text)
{
    std::vector<std::u8string> reversed_text = ReverseFAText(text);
    std::u8string convertedText;
    std::u8string previous_fachar, next_fachar;
    ...
    return (char *)convertedText.c_str();
}